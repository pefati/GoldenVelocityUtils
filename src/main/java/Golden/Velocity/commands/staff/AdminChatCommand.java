...
public class AdminChatCommand {
    ...
    public BrigadierCommand createCommand() {
        LiteralCommandNode<CommandSource> node = LiteralArgumentBuilder
            ...
            .requires(source -> source.hasPermission("gbu.admin.chat"))
            .then(RequiredArgumentBuilder.<CommandSource, String>argument("message", StringArgumentType.greedyString())
                .executes(context -> {
                    ...
                    Component adminMessage = plugin.getChatManager().formatAdminChatMessage(player, message);
            ...
            .executes(context -> {
                ...
                Player player = (Player) context.getSource();
                plugin.getChatManager().toggleAdminChat(player.getUniqueId());
                boolean enabled = plugin.getChatManager().isInAdminChat(player.getUniqueId());
                player.sendMessage(Component.text(enabled ? "§aAdminChat mode enabled!" : "§cAdminChat mode disabled!"));
                
                sendToggleUpdate(player, "AdminChat", enabled);
                return 1;
            })
            .build();
